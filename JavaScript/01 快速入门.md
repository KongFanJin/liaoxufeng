...
第二种方法是把JavaScript代码放到一个单独的`.js`文件，然后在HTML中通过``引入这个文件：
...
可以在同一个页面中引入多个`.js`文件，还可以在页面中多次编写` js代码... `，浏览器按照顺序依次执行。

有些时候你会看到``标签还设置了一个`type`属性：
...
![chrome-HOWTO](https://www.liaoxuefeng.com/files/attachments/1025934180217184/l)
